...
interpolation at level $k$ is $O\left(\binom{d + k}{k}^2\right)$ (see \autoref{polynomial}).  Thus, the expected cost of the perturbation scheme is
...
to degree 12 as discussed in \cite{devillers2000algebraic}, or to degree 6 using a combination of polynomial factoring and algorithmic changes (see \autoref{predicates}).  Even
...
\section{Degree 6 circular arc predicates} \label{predicates}

Our current implementation of circular arc polynomial Booleans predicates up to the degree 20.  Although our method handles high degree predicates without difficulty, they are
naturally slower, and it is natural to look for lower degree versions.  Indeed, \cite{devillers2000algebraic} show that the critical predicate, comparing the $y$ coordinates of
two intersections of two different pairs of circles $C_0,C_1$ and $C_2,C_3$, can be reduced to degree 12 using resultant techniques.  We believe this is optimal for that predicate, since the resulting
polynomial is irreducible over $\Q$.  However, to compute a circular arc arrangements it suffices to consider the case where $C_0 = C_2$; that is, comparing the $y$ coordinates
of the intersections of one circle with two others.  In this case, the polynomials can be factored into terms with degree $\le 6$.  One significant algorithmic change is required,
...
rays from exactly known (degree 1) $y$ coordinates, which is sufficient to determine the winding number of a given circular arc polygon (or connected component of an arrangement)
as long as the bounding box touches at least one such ray.  For most applications, polygons smaller than this may be safely discarded.
